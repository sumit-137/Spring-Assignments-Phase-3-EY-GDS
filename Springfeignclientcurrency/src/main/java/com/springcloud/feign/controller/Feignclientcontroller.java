...
// Application url - localhost:9192/feign/getCurrency/2
...
public class Feignclientcontroller {
	 ...
	 * @return
	 */
	@GetMapping(value="/getCurrency/{amt}", produces= MediaType.APPLICATION_JSON_VALUE)
	...
	public ResponseEntity<String> defaultResponse(String err) {
		...
		err = "Not able to convert the currency as microservice is down";
		return new ResponseEntity<String>(err, HttpStatus.INTERNAL_SERVER_ERROR);
	}
	
	@GetMapping(value="/getCurrency/{amt}", produces= MediaType.APPLICATION_JSON_VALUE)
	@HystrixCommand(fallbackMethod= "INRtoUSDResponse")
	public ResponseEntity<String> getGreetingsAndUserInfoViaFeign1(@PathVariable(name= "localeId") String langCode) {
		System.out.println("Using the feign client controller to fetch the greetings information for locale= " + langCode);
	...
	// When we define a fallback-method, the fallback-method must match the same parameters of the method where you define the Hystrix Command using the hystrix-command annotation.
	public ResponseEntity<String> INRtoUSDResponse(String err) {
		System.out.println("You are seeing this fallback response because the underlying microservice is down.");
		err = "Not able to convert the from inr to usd as microservice is down";
		return new ResponseEntity<String>(err, HttpStatus.INTERNAL_SERVER_ERROR);
	}
	
	@GetMapping(value="/getCurrency/{amt}", produces= MediaType.APPLICATION_JSON_VALUE)
	@HystrixCommand(fallbackMethod= "EuroToYenResponse")
	public ResponseEntity<String> getGreetingsAndUserInfoViaFeign2(@PathVariable(name= "localeId") String langCode) {
		System.out.println("Using the feign client controller to fetch the greetings information for locale= " + langCode);
	...
	// When we define a fallback-method, the fallback-method must match the same parameters of the method where you define the Hystrix Command using the hystrix-command annotation.
	public ResponseEntity<String> EuroToYenResponse(String err) {
		System.out.println("You are seeing this fallback response because the underlying microservice is down.");
		err = "Not able to convert the currency from Euro to yen as microservice is down";
		return new ResponseEntity<String>(err, HttpStatus.INTERNAL_SERVER_ERROR);
	}
	
	@GetMapping(value="/getCurrency/{amt}", produces= MediaType.APPLICATION_JSON_VALUE)
	@HystrixCommand(fallbackMethod= "EuroToInrResponse")
	public ResponseEntity<String> getGreetingsAndUserInfoViaFeign3(@PathVariable(name= "localeId") String langCode) {
		System.out.println("Using the feign client controller to fetch the greetings information for locale= " + langCode);
	...
	// When we define a fallback-method, the fallback-method must match the same parameters of the method where you define the Hystrix Command using the hystrix-command annotation.
	public ResponseEntity<String> EuroToInrResponse(String err) {
		System.out.println("You are seeing this fallback response because the underlying microservice is down.");
		err = "Not able to convert the currency from Euro to inr as microservice is down";
		return new ResponseEntity<String>(err, HttpStatus.INTERNAL_SERVER_ERROR);
	}
}